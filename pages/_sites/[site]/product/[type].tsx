...
import SliderSportEvent from "@components/games/sports/slider-sport-event";
import SliderSportbook from "@components/games/sports/slider-sportbook";
...
import { useSportEventQuery } from "@framework/game/get-all-providers";
...
const Product = ({ config }: DefaultPageProps) => {
  ...
  const { data: sportEvents, isLoading: eventLoading } = useSportEventQuery({
    enabled: pageType === "sport",
    start_date: new Date().toISOString().replace(/T/g, " ").split(".")[0],
  });
  ...
  const getProviderLogos = () => {
    ...
    const uniqueProviders = [...new Set(listProviders)];
  ...
  return (
    <div className="container mx-auto px-mobile px-desktop lg:mt-10">
      <CarouselSwiperBanner page={pageType} data={dataBanner} />
      <div className="flex flex-col">
        <div className="order-3 md:order-1">
          {isAuthorized && <SliderRecentPlayed margin={false} />}
        </div>
        <div className="order-2 md:order-3 mt-[30px] md:mt-10 pb-0">
          <ProductTitle
            pageType={pageType}
                    ...
                    {getProviderLogos()?.map((provider: string, i: number) => {
                      return (
                        ...
                        >
                          ...
                          {provider.toLowerCase() === "pragmatic" && (
                            <div className="absolute z-[1] -left-[1px] -top-[1px]">
                              <Image
                                alt=""
                                src="/images/provider/pragmatic/hot.png"
                                width={83}
                                height={20}
                                quality={100}
                                objectFit="contain"
      ...
      {pageType === "sport" && (
        <>
          <SliderSportEvent
            ...
            data={sportEvents?.data}
          />
          <SliderSportbook
            title="sportbook"
            isLoading={isLoading_game_type}
            data={gameCountByType?.[0]?.providers}
          />
        </>
      )}
