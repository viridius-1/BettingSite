...
import IconHeaderSlot from "public/icons/header/slot.svg"
import IconHeaderCasino from "public/icons/header/casino.svg"
import IconHeaderSports from "public/icons/header/sports.svg"
import IconHeaderArcade from "public/icons/header/arcades.svg"
import IconHeaderPoker from "public/icons/header/poker.svg"
import IconHeaderTogel from "public/icons/header/togel.svg"
import IconHeaderHome from "public/icons/header/home.svg"
  ...
  return (
    <div className="flex flex-row space-x-[1px]">
      <a
        ...
        className={`${handleActiveMenu(t("common:text-home") as string)} space-x-2 cursor-pointer active:translate-y-1 transition-all duration-200 ease-in-out`}
        target="_self"
        rel="noreferrer"
      >
        {/* <div className={`${handleActiveMenuIcons(t("common:text-home") as string)} w-9 h-9 rounded-full justify-center`}> */}
          <IconHeaderHome className="icon_subheader" />
        {/* </div> */}
      ...
      {listGame.map((item, index) => (
        <a
          ...
          className={`${handleActiveMenu(item.name)} space-x-2 cursor-pointer  active:translate-y-1 transition-all duration-200 ease-in-out`}
        // target={item.name === "togel" ? "_blank" : "_self"}
        // rel="noreferrer"
        >
          {/* <div className={`${handleActiveMenuIcons(item.name)} w-9 h-9 rounded-full justify-center`}> */}
            <item.icon className="icon_subheader" />
          {/* </div> */}
          <div className="flex flex-col relative group">
            <span>
              {item.name}
      ...
      <a
        ...
        className={`${router.asPath === "/result-togel" ? `submenu_item_active` : `submenu_item`} space-x-2 cursor-pointer px-[10px] active:translate-y-1 transition-all duration-200 ease-in-out`}
      >
        {/* <div className={`${router.pathname === "/result-togel" ? "bg-transparent" : "bg-black/20"} w-9 h-9 rounded-full justify-center`}> */}
          <IconHeaderTogel className="icon_subheader" />
        {/* </div> */}
        <div className="flex flex-col relative group">
          <span>
            togel
