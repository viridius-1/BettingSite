...
const ListSubMenu = () => {
  ...
  const handleActiveMenu2 = (name: string) => {
    ...
    if (checkPath) return `submenu_item2_active`;
      ...
      <div className="flex flex-row space-x-[1px] text-base font-medium uppercase">
        {token &&
          <a
            ...
            className={`${handleActiveMenu2("/transaction")} space-x-2 cursor-pointer  active:translate-y-1 transition-all duration-200 ease-in-out`}
          >
        ...
        {!token && getLinkAlternative?.[0]?.value !== "-" &&
          <a
            ...
            className={`submenu_item space-x-2 cursor-pointer active:translate-y-1 transition-all duration-200 ease-in-out`}
          >
        ...
        <a
          href={"/promo"}
          className={`${handleActiveMenu2("/promo")} space-x-2 cursor-pointer active:translate-y-1 transition-all duration-200 ease-in-out`}
        >
