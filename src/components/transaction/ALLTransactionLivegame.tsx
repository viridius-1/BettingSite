...
const ALLTransactionLivegame = (props: any) => {
    ...
    return DataFromFilterType?.map(
      (value: { active: boolean; name: string }, i) => {
        if (value.active == true) {
          return (
            <div
              className={`bg_content_container px-4 mr-[10px] mb-[10px] py-2 text-[#d3f2ff80] flex items-center rounded-[4px]`}
              key={i}
            >
  ...
  return (
    <div className={`bg_content_container w-full md:rounded-10 text-white py-6 md:px-0 md:py-0 mb-4`}>
      {isLoading && <Indicator />}
      <div className="md:px-10 md:py-[30px]">
        ...
        {DataFilterCount > 0 || DataFilterCount2 > 0 ? (
          <div className="w-full px-mobile md:px-0 flex-row sm:flex-row flex flex-wrap mt-[1rem]  overflow-hidden overflow-x-auto items-center">
            ...
            {RenderFilter()} {RenderFilterProfider()}
            <>
              <div
                onClick={() => setClearButtonData()}
...
export default ALLTransactionLivegame;
