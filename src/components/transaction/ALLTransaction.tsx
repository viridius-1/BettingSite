...
const ALLTransaction = (props: any) => {
    ...
    return DataFromFilterProvider?.map(
      (value: { active: boolean; name: string }, i: number) => {
        if (value.active == true) {
          return (
            <div
              className={`bg_content_container h-[38px] px-4 mr-[10px] mb-[10px] text-[#d3f2ff80] flex items-center space-x-2 rounded-[4px]`}
              key={i}
            >
              <Image
  ...
  const columns: Array<Column<object>> = [
    ...
    {
      Header: () => (
        <div className="styleHeader  justify-center text-center">
          {" "}
          {t(`text-type`)}
        </div>
      ),
      ...
      Cell: ({ value }) => {
        return (
          <div className="styleRow text-whiteDefault-100 text-center text-[14px]">
            {value.replaceAll("_", " ").slice(0, 8)}
          </div>
        );
      },
    },
  ...
  return (
    <div className={`bg_content_container flex flex-col md:rounded-10 w-full text-white py-6 md:px-0 md:py-0`}>
      {isLoading && <Indicator />}
      ...
      {members && members.length !== 0 ? (
        ...
        <div className="w-full text-center pt-[2rem] pb-[2rem] text-sm font-semibold text-whiteDefault-40">
