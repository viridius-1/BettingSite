  ...
  return (
    <div className="flex flex-col lg:flex-row w-full">
      ...
      >
        <div className="hidden lg:block absolute z-[1] -top-[50px] lg:-top-[228px] right-0 lg:-right-[240px]">
          <Image
            ...
            quality={100}
            src={`/images/theme/${template}/bg-featured-game-1.png`}
            objectFit="contain"
            objectPosition="left"
          />
        </div>
        <div
          className={`absolute lg:hidden z-[1] w-2/3 aspect-square -top-[50px] md:-top-[100px] right-0 md:-right-[100px] bg-contain bg-no-repeat bg-bottom`}
        >
          <Image
            alt=""
            width={device.isMobileDevice ? 317 : 500}
        ...
        <div className="relative z-[2]">
          <h1 className="text-[32px] md:text-[44px] leading-[32px] md:leading-[44px] font-light uppercase max-w-[189px] md:max-w-[350px]">
            {t("common:text-featured-games-header")}
          </h1>
          <h5 className="text-sm font-normal md:text-base mt-[7px] md:mt-3 max-w-[260px] md:max-w-[300px] normal-case">
            {t("common:text-featured-sub-header")}
          </h5>
          <Button
            type="submit"
            className="flex flex-row w-fit h-11 font-extrabold text-sm px-5 rounded-md text-[#07243A] bg-white hover:bg-[#e3e3e3] active:translate-y-[2px] transition-all duration-200 ease-in-out mt-7 md:mt-[34px] uppercase"
            onClick={() => {
              router.push({
                pathname: "/gamelist",
                query: {
      ...
      <div className="mt-[34px] md:mt-[29px] flex-1">
        {!device.isTabDevice ? (
          <CarouselSwiperCustom title="featured" data={data?.data} />
        ) : (
