...
import IconSearch from "public/icons/search-custome.svg";
import IconFilter from "public/icons/filter-custome.svg";
...
const SearchGame = ({
  ...
  const handleApplyFilter = () => {
    if (localSearch) {
      ...
      if (isProductPage) {
        pushParams = {
          pathname: "/product/" + urlQuery?.type,
        };
      }
  ...
  return (
    ...
    >
      <div className={`flex ${alignItem} items-center w-full gap-[25px]`}>
        {showSearch ? (
          <Combobox>
            <div className="">
              <form
                autoComplete="off"
                className={cn(
                  "flex flex-1 relative items-center transition duration-200 h-[44px] md:h-[53px] mt-[1px] md:mt-0",
                  style_form,
                  {
              ...
              >
                <Combobox.Input
                  className={`${inputWidth} bg-transparent outline-none h-full pl-5 pr-10 capitalize focus:placeholder:text-transparent`}
                ...
                <div className="absolute right-0 top-0 w-[65px] h-full flex items-center justify-center border-l border-r border-[#1E2834] ">
                  <IconSearch className="icon_search w-5 h-5" />
                </div>
              </form>
              <Transition
                as={Fragment}
              ...
              >
                <Combobox.Options
                  className={`bg_search_game absolute z-50 ${searchPosition} px-5 py-4 mt-5 max-h-96 overflow-auto scrollbar text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm w-full sm:w-[425px] rounded-[10px]`}
                >
        ...
        {showFilter ? (
          <Menu as="div">
            <Menu.Button
              ...
              className={cn(
                `flex gap-[5px] items-center justify-center md:mt-0 h-[44px] md:h-[53px] py-[10px] px-[12px] text-sm active:text-[13px] transition duration-200 ease-in-out`,
                {
            ...
            >
              ...
              <IconFilter className="icon_search cursor-pointer w-5 h-5" />
            </Menu.Button>
            <Transition
              as={Fragment}
                      ...
                      <div className="flex flex-wrap gap-2 flex-row items-start justify-start overflow-y-scroll scrollbar max-h-40">
                        {listProviders.map(
                          (
                            ...
                            <button
                              key={index}
                              className={`flex space-x-1 h-10 flex-row uppercase items-center justify-center py-3 px-3 rounded-md ${isSelectedProvider(item.key)
                                  ? "bg_filter_item_active"
                                  : "bg_filter_item"
                                } text-sm text-center cursor-pointer  active:translate-y-1 transition-all duration-200 ease-in-out`}
                              onClick={() => {
