...
const Datatable = ({
  ...
  return (
    <div className={`bg_cont_web rounded-[5px] w-full text-sm font-semibold text-whiteDefault-40`}>
      <div className="overflow-x-auto scrollbar rounded-t-[5px] -mx-[1px] -mt-[1px]">
        <table
          {...getTableProps()}
          className={classNames("table", tableClassName)}
        >
            {isEmpty(data) && !isLoading ? null :  <thead>
              ...
              >
                {headerGroup.headers.map((column) => (
                  <th
                    ...
                    className="px-4 pt-[30px] pb-[10px] text-white text-lg"
                  >
          ...
          <tbody {...getTableBodyProps()}>
            {isEmpty(data) && !isLoading ? (
                  <tr>
                  <td colSpan={columns.length} className="!text-center pt-[2rem] pb-[2rem]">
